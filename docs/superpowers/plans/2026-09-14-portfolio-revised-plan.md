...
| A39 | **A36's caveat is resolved by taking its first option: in a room the rig now swings to sit behind the player.** `CameraRig` takes `inRoom` and `startYaw`; the yaw is damped at lambda 3.5 (its own clock, not the character's lambda 12) and the aim moves with it. The hub keeps a fixed yaw. | This is the fix A36 named but did not take: "the wall frames are on *side* walls, so a fixed south-facing camera always views them at a grazing angle. That, not distance, is the main reason wall copy is hard to read; fixing it properly means either rotating the camera with the player or moving the frames to the north wall." Measured 2026-09-17 on the built site at 900x560: standing at Nightfall's west wall by the plaque, facing **west** gives the frame 20,111 px on screen (aspect 1.11, 4,762 px of text/border ink) against 4,814 px / aspect 1.67 for the same tile facing **north** - and that north-facing shot is a thin sliver with the copy unreadable. The hub, and a room's north-facing view, are unchanged from `dd38dda`, so the swing costs the menu nothing. |
| A40 | The rig's start yaw is **passed in** (`startYaw`, the room's spawn facing), never read off the player group on the first frame | Found while verifying A39, and it was a real bug in the first cut of it. `CameraRig` subscribes to `useFrame` before `PlayerController` does, so on the frame the rig mounts the group still carries its default rotation. A temporary log measured it: frame 0 saw `groupY=0.000` while the spawn faced `3.142`. That first cut therefore adopted south, and every entry into a north-facing room opened with a 180-degree swing before settling - visible in the probe frames as sloping belt rails at t+1.4s and level ones by t+4.5s, and confirmed by restoring the old line (yaw 0.000 climbing through 2.009 to 2.443 over ten frames). Passing the spawn facing in removes the ordering dependency. `ponytail:` the swing is a full follow with no dead zone, so tapping left-right swings the whole room; if that ever reads as jumpy the upgrade is a hysteresis band, not a slower constant. |
